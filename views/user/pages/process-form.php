...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $insertQuery = "INSERT INTO treiven_orders (treiven_id, trv_category_name, trv_total_amounts, shipping_address, shipping_method, payment_method, trv_createdAt, trv_item_boxes, trv_customer_name, trv_contact_number, trv_ref_number, trv_order_number, trv_status_id, trv_item_names) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($insertQuery);

    $stmt->bind_param(
        "isssssssssssss", $treiven_id, $trv_category_name, $trv_total_amounts, $shipping_address,
        $shipping_method, $payment_method, $trv_createdAt, $trv_item_boxes, $trv_customer_name,
        $trv_contact_number, $trv_ref_number, $trv_order_number, $trv_status_id, $trv_item_names
    );
    ...
    if (mysqli_query($conn, $deleteQuery)) {
        echo "All existing values deleted successfully from treiven_cart_items.";
    } else {
